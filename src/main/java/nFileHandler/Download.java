	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(Validate.verifyToken(request.getParameter("usertoken")  , username) == 1 ){
			...
			if (userInfo != null){
				...
				//Check the sharing permissions to ensure that user has permission to file before allowing them to download
				if (ShareFile.validateUserPermission(userID, fileID) == 1){
					...
					try{
						...
						if( getFilePath(fileModel) ){
				            ...
				            Log.log("Download Process| "+ username + " downloaded " + fileModel.getFileName() + ", size :" +fileModel.getFileLength() + "bytes");
						}
